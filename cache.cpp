...
bool Cache::isHit(char *addrBin, int setIndex) {
    ...
    if(DEBUG){
        cout<<"address tag: "<<address.substr(0,11)<<"\tindex: "<<address.substr(11,17)<<"\toffset: "<<address.substr(28,4)<<endl;
    }
    for(int i=0;i<Way_Associative;++i){
        ...
        string addrTag=addrBin;
        addrTag = addrTag.substr(0,11);
        string cacheTag = tag;
        cacheTag = cacheTag.substr(0,11);
        a=!strcmp(cacheTag.c_str(),addrTag.c_str());
